...
//   KEY line (tip) to J8 pin 2 (GPIO18_In2) with external 4.7K ohm pull-up resistor to 3.3V (3V2 on IO Board)
...
//   9. Safety timeout of 15 seconds prevents indefinite transmission
//
// Error codes:
//   0x00 - Success
...
void IcomAh4(uint8_t AH4_START, uint8_t AH4_KEY)
{
    ...
    case 3:     // RF active - wait for KEY to go high indicating tuning complete
                // Safety timeout of 15 seconds prevents indefinite transmission
        if (gpio_get(AH4_KEY)) {
            Registers[REG_ANTENNA_TUNER] = 0;      // Tuning complete - instruct Thetis to stop RF
            ldg_state = 0;                          // Return to idle
        }
        else if (absolute_time_diff_us(ldg_timer, get_absolute_time()) / 1000 >= 15000) {
